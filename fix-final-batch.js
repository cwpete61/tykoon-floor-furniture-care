...
// Final batch of location pages to fix
const finalBatch = [
...
finalBatch.forEach(filePath => {
  if (fs.existsSync(filePath)) {
    try {
      let content = fs.readFileSync(filePath, 'utf8');

      // Replace the entire template with proper structure
      content = content.replace(/^\s*<body[^>]*>[\s\S]*<\/body>/m, `<body class="font-['Inter'] antialiased bg-black text-white">
    <main>
        <section class="relative h-[400px] flex items-center justify-center" style="background-image: url('../../../images/heroes/locations/${path.basename(path.dirname(path.dirname(filePath)))}/${path.basename(path.dirname(filePath))}/hero.jpg'); background-size: cover; background-position: center;">
